...
class Role extends CI_Controller {
    ...
    public function listRole() {
        ...
        $data['arr_roles'] = $this->common_model->getRecords('mst_role','','','role_id desc');
    ...
    public function addRole($edit_id = '') {
       ...
       if (count($this->input->post()) > 0) {
		    ...
		    $this->session->set_userdata('msg', "<span class='success'>Global setting parameter updated successfully!</span>");
                   /* deleting the role old priviliges */
                    $this->common_model->deleteRows(array(base64_decode($this->input->post('edit_id'))), "trans_role_privileges", "role_id");
                    /* inserting the role new priviliges */
                    $role_privileges = $this->input->post('role_privileges');
                    foreach ($role_privileges as $privilege) {
                        $privilege_to_insert = array("role_id" => base64_decode($this->input->post('edit_id')), "privilege_id" => $privilege);
                    ...
                    $arr_to_insert = array("role_name" => mysql_real_escape_string($this->input->post('role_name')));
