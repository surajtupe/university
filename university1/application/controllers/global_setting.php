...
class Global_Setting extends CI_Controller {
    ...
    public function editGlobalSettings($edit_id = '', $lang_id = '') {
        ...
        $data['title'] = "Update Global Settings";
        if (($edit_id != '') && ($lang_id != '')) {
    ...
    public function editParameterLanguage($edit_id = '') {
        ...
        if ($this->input->post('lang_id') != '') {
            if ($this->input->post('value') != "") {
                if ($this->input->post('edit_id') != '' && $this->input->post('lang_id') != '') {
                    ...
                    $insert_array = array('lang_id' => intval($this->input->post('lang_id')), 'global_name_id' => intval(base64_decode($this->input->post('edit_id'))),"value" => mysql_real_escape_string($this->input->post('value')));
                    
                    /*** check if record is already exist **/
                    $arr_global_setting=$this->common_model->getRecords('trans_global_settings','', $condition_array);
                   ...
                   if(count($arr_global_setting)>0)
                   {
                    ...
                    $this->common_model->insertRow($insert_array,'trans_global_settings');   
                   }
        ...
        if (($edit_id != '')) {
            ...
            $data['arr_languages'] = $this->common_model->getAllLanguages();
